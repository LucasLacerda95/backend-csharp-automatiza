...
namespace crud.API.Controllers
{
    ...
    public class ProdutosController : ControllerBase
    {
        ...
        public async Task<ActionResult<ProdutoViewModel>> Adicionar([FromBody] ProdutoViewModel produtoViewModel)
        {
            ...
            if (produtoViewModel.Estoque < 0) return BadRequest("A propriedade estoque não pode ser menor que 0");
            if (produtoViewModel.Preco < 0) return BadRequest("A propriedade preço não pode ser menor que 0");

            await _produtoService.Adicionar(_mapper.Map<Produto>(produtoViewModel));//Binding de uma view Model em uma Entidade Entity

            return Ok($"Produto {produtoViewModel.Descricao} foi adicionado com êxito!");
        }
        ...
        public async Task<IActionResult> Atualizar([FromRoute] Guid id, [FromBody] ProdutoViewModel produtoViewModel)
        {
            ...
            return Ok($"Produto {produtoViewModel.Descricao} foi atualizaco com êxito !");
        }

        [HttpDelete("{id:guid}")]
        public async Task<ActionResult<ProdutoViewModel>> Excluir([FromRoute] Guid id)//SoftDelete
        {
            ...
            return Ok("Produto deletado com êxito!");
        }
